...
const SidebarItem = ({ icon, label, shortcut, active, to, collapsed = false, onClick }: SidebarItemProps) => {
  ...
  if (to) {
    ...
    return collapsed ? <Hint label={label}>{link}</Hint> : link;
  }
  ...
  return collapsed ? <Hint label={label}>{item}</Hint> : item;
};
...
const ThemeQuickToggle = () => {
  ...
  return (
    <Hint label={isDark ? "Switch to light theme" : "Switch to dark theme"}>
      <button
        type="button"
        onClick={toggle}
...
export const Sidebar = ({ collapsed = false, onToggleCollapse, onNewChat }: SidebarProps) => {
  ...
  // True conversation count for the destructive confirm (the loaded list
  // is page-capped at 50 — audit M3).
  ...
  const handleOpenDeleteAllConfirm = () => {
    ...
    void getConversationsCount()
      .then(({ count }) => setDeleteAllCount(count))
  ...
  const loadConversations = async (cursor?: string, query?: string) => {
    try {
      const data = await getConversations(cursor, query);
      if (cursor) {
      ...
      // A transient poll error must not wipe the visible list (audit M3):
      // keep whatever is on screen; only a first-ever load starts empty.
      if (!cursor && conversations.length === 0) setConversations([]);
    }
  };
  ...
  // Server-side search: fires on the debounced query so results are not
  // limited to the pages the client happens to have loaded (audit M3).
  // While the round-trip is in flight, the local filter keeps typing
  // feeling instant.
  const [serverSearchResults, setServerSearchResults] = useState<ConversationListItem[] | null>(null);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    setSearching(true);
    getConversations(undefined, query)
      .then((data) => { if (!cancelled) setServerSearchResults(data.items); })
  ...
  const filteredConversations = useMemo(() => {
    ...
    return conversations.filter((chat) => (chat.title || "Untitled").toLowerCase().includes(lower));
  ...
  const groupedConversations = useMemo(() => {
    ...
    for (const chat of filteredConversations) {
      ...
      if (time >= startOfToday) groups[0].items.push(chat);
      else if (time >= startOfYesterday) groups[1].items.push(chat);
      else if (time >= weekAgo) groups[2].items.push(chat);
  ...
  const handleConfirmRename = async () => {
    ...
    try {
      ...
      // The dialog previously stayed open with no feedback on failure
      // (audit M3) — surface the error and close so the user can retry.
  ...
  const handleDeleteAll = async () => {
    ...
    try {
      ...
      toast.success(`Deleted ${result.count} conversation${result.count === 1 ? "" : "s"}`);
                ...
                {conversations.length > 0 && (
                  <DropdownMenu.Root>
                    ...
                    <DropdownMenu.Portal>
                      ...
                      >
                        <DropdownMenu.Item
                          onSelect={handleOpenDeleteAllConfirm}
                ...
                <div className="space-y-0.5 pb-3">
                    {filteredConversations.length === 0 ? (
                      ...
                      groupedConversations.map((group) => (
                        <div key={group.label}>
                          <div className="px-3 pb-0.5 pt-3 font-mono-tech text-[9px] font-semibold uppercase tracking-[0.14em] text-muted-foreground/50">
                            {group.label}
                          </div>
        ...
        <AlertDialog open={showDeleteAllConfirm} onOpenChange={(open) => {
          ...
          <AlertDialogContent className="dialog-panel rounded-lg text-foreground">
            <AlertDialogHeader>
              ...
              <AlertDialogDescription className="text-[10px] text-muted-foreground">
                Are you sure you want to delete all {deleteAllCount ?? "…"} conversation{deleteAllCount === 1 ? '' : 's'}? This action cannot be undone.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              <AlertDialogCancel
                      ...
                      >
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-1.5">
                            ...
                            {runningCount > 0 ? (
                              <Hint label={`${runningCount} agent${runningCount === 1 ? "" : "s"} running in this workspace`}>
                                <span
                            ...
                            {pendingCount > 0 ? (
                              <Hint label={`${pendingCount} tool approval${pendingCount === 1 ? "" : "s"} waiting in this workspace`}>
                                <span
